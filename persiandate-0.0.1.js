...
function _getTokenReplacement(token, date, lang) {
    if (lang == 'fa') {
        switch (token) {
            case 'd':
                return date.toJalali()[2];
            case 'dd':
                return _zeroPad(date.toJalali()[2]);
            case 'ddd':
                return locales.fa.dayNamesShort[date.jGetDay() - 1];
            case 'dddd':
                return locales.fa.dayNames[date.jGetDay() - 1];
            case 'M':
                return date.toJalali()[1];
            case 'MM':
                return _zeroPad(date.toJalali()[1]);
            case 'MMM':
                return locales.fa.monthNamesShort[date.toJalali()[1] - 1];
            case 'MMMM':
                return locales.fa.monthNames[date.toJalali()[1] - 1];
            case 'yy':
                return parseInt((date.toJalali()[0] + '').substring(2));
            case 'yyyy':
                return date.toJalali()[0];
        }
    }
}
